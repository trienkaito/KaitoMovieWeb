...
using MoviesWebsite.Areas.Identity.Models.RoleViewModels;
...
namespace MoviesWebsite.Areas.Identity.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> CreateAsync(CreateRoleModel model)
        {
            ...
            if (result.Succeeded)
            {
                Messages = $"You just create new role: {model.Name}";
        ...
        public async Task<IActionResult> DeleteConfirmAsync(string roleid)
        {
            ...
            if (result.Succeeded)
            {
                Messages = $"You just delete: {role.Name}";
                return RedirectToAction(nameof(Index));
            }
            else
            {
        ...
        public async Task<IActionResult> EditConfirmAsync(string roleid, [Bind("Name")] EditRoleModel model)
        {
            ...
            if (result.Succeeded)
            {
                Messages = $"You just change role name: {model.Name}";
                return RedirectToAction(nameof(Index));
            }
            else
            {
        ...
        public async Task<IActionResult> AddRoleClaimAsync(string roleid, [Bind("ClaimType", "ClaimValue")] EditClaimModel model)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Claim already have in role");
                return View(model);
            }
